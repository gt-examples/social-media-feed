...
function TrendingTopic({ tag, postCount }: { tag: string; postCount: number }) {
  return (
    <T>
      <div className="border border-neutral-800 rounded-lg p-3 hover:border-neutral-700 transition-colors">
        <p className="text-neutral-200 font-medium text-sm">
          #<Var>{tag}</Var>
        </p>
        <p className="text-neutral-500 text-xs mt-1">
          <Num>{postCount}</Num>{" "}
          <Plural singular="post" plural="posts" n={postCount} />
        </p>
      </div>
    </T>
  );
...
function SuggestedUser({
  ...
  return (
    <T>
      <div className="border border-neutral-800 rounded-lg p-4 flex items-center gap-3">
...
export default async function Explore() {
  ...
  const suggestedBios: Record<string, string> = {
    "@priyasharma": gt("Localization engineer at a global e-commerce company."),
    "@jlmartin": gt("French-Canadian developer passionate about accessible i18n."),
    "@aishabello": gt("Open source contributor focused on African language support."),
  };
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          {trendingTopics.map((topic) => (
            <TrendingTopic key={topic.tag} tag={topic.tag} postCount={topic.postCount} />
          ))}
        </div>
        ...
        <div className="space-y-3 mb-8">
          {suggestedUsers.map((user) => (
            <SuggestedUser
              key={user.handle}
              user={user}
              translatedBio={suggestedBios[user.handle] ?? ""}
            />
          ))}
        </div>
